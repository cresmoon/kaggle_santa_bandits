...
class MLEUCBAgentV3:
    ...
    def __call__(self, obs: Observation):
        ...
        if self.is_first_phase:
            ...
            # do naive exploitation here
            if (last_arm is not None) and (net_reward > 0):
                act = last_arm
                return act
            act = self.first_phase_arm[self.first_phase_arm_idx % self.num_arms]
            self.first_phase_arm_idx += 1
            if self.first_phase_arm_idx >= self.num_arms * NUM_1ST_PHASE_ROUNDS:
                # end of first phase
...
def agent(raw_observation, raw_configuration):
    ...
    if cached_agent is None:
        ...
        cached_agent = MLEUCBAgentV3(configuration)
